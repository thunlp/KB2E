...
private:
    ...
    void bfgs()
    {
        ...
        for (int eval=0; eval<neval; eval++)
        {
            ...
            fclose(f2);
            fclose(f3);
			FILE* f1 = fopen(("W.txt"+version).c_str(),"w");
			for (int i=0; i<2*n; i++)
			{
				for (int j=0; j<n; j++)
					fprintf(f1,"%.6lf\t",W[i][j]);
				fprintf(f1,"\n");
			}
			fclose(f1);
        }
    ...
    void gradient_path(int r1,vector<int> rel_path, double belta)
    {
		...
		delta.resize(num_l+1);
		delta[num_l-1].resize(n);
		for (int i=0; i<n; i++)
		{
			//cout<<i<<endl;
			double tmp = relation_vec[r1][i]-a[num_l-1][i];
			...
			if (L1_flag)
		    	if (tmp>0)
					...
					tmp = -1;
			//cout<<i<<endl;
			delta[num_l-1][i] = tmp*f_derivative(z[num_l-1][i]);//to be add tanh
			//cout<<i<<endl;
			relation_tmp[r1][i]+=rate*belta*tmp;
			//cout<<i<<endl;
		}
		//cout<<362<<endl;
		for (int l=num_l-2; l>=0; l--)
		{
			for (int i=0; i<n; i++)
			{
				for (int j=0; j<n; j++)
				{
					W_tmp[j][i]-=rate*belta*delta[l+1][i]*a[l][j];
					W_tmp[j+n][i]-=rate*belta*delta[l+1][i]*relation_vec[rel_path[l+1]][j];
					relation_tmp[rel_path[l+1]][j]-=rate*belta*delta[l+1][i]*W[j+n][i];
					if (l==0)
						relation_tmp[rel_path[0]][j]-=rate*belta*delta[l+1][i]*W[j][i];
				}
			}
			if (l>0)
			{
				...
				for (int i=0; i<n; i++)
					for (int j=0; j<n; j++)
						delta[l][i]+=W[i][j]*delta[l+1][j]*f_derivative(z[l][i]);
			}
		}
		//cout<<384<<endl;
    }
